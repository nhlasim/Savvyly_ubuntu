...
def search_users_account_number(request):
    # account = Account.objects.filter(account_status="active") #'filter (instead of all) allows you to get the accounts of people that are 'active'
    account = Account.objects.all()
    query = request.POST.get("account_number") # 2176669228578; 2174491756406
    if query:
        account = account.filter(
            Q(account_number=query)|
            Q(account_id=query) #could put email but perhaps make sure it included in account/models.py
        ).distinct()
...
def AmountTransferProcess(request, account_number):
    ...
    if request.method == "POST":
        ...
        if sender_account.account_balance >= Decimal(amount):
            ...
            # Get the id of the transaction that vas created now
    ...
    else: #this prevents cross site forgery and makes sure the method is post
